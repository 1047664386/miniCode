...
export async function gitCommit(
  ...
  // 写 message 到临时文件以避免 shell 注入（使用 os.tmpdir() 跨平台兼容）
  const file = path.join(os.tmpdir(), `.minicodeide-commit-${Date.now()}.txt`);
...
/** 撤回指定文件到 HEAD 版本（git checkout -- <path>） */
export async function gitRevert(cwd: string, filePath: string): Promise<void> {
  const safe = filePath.replace(/'/g, "'\\''");
  await run(`git checkout -- '${safe}'`, cwd);
}

/** 暂存 + 提交指定文件（git add + git commit） */
